...
db <- read.table(file="Jaturapruek_et_al_database.csv", sep=',', dec='.', header=T, as.is=F)
str(db)
dim(db)
names(db)
summary(db)
...
names(db_reduced)
Hmisc::hist.data.frame(db_reduced[,c(9:14,25:33)])

# transform in log scale CONDUCTIVITY
summary(db_reduced)
...
PerformanceAnalytics::chart.Correlation(db_reduced[,c("PREC",
                              ...
                              "CONDUCTIVITY")])
# Not collinear:
# CONDUCTIVITY, DO, pH, TEMPERATURE, MEAN_AIR_TEMP, PREC, SRAD
...
db_analyses <- data.frame(lat = db_reduced$LATITUDE,
                          ...
                          habitat =  db_reduced$hab,
                          conductivity = db_reduced$CONDUCTIVITY,
                          DO = db_reduced$DO,
                          temperature = db_reduced$TEMPERATURE,
                          pH = db_reduced$pH,
                          srad = db_reduced$SRAD,
                          air_temperature = db_reduced$MEAN_AIR_TEMP,
                          precipitation = db_reduced$PREC,
                          ...
                          monogononta = db_reduced$Monogononta,
                          cladocera = db_reduced$Cladocera,
                          copepoda = db_reduced$Copepoda)
...
plot(hvBoth,
     ...
     show.centroid=TRUE,
     cex.centroid=2,
     names= c("Conductivity", "Dissolved oxygen", "Temperature","pH","Solar radiation","Air temperature","Precipitation"))
dev.off()
...
db2 <- read.table(file="RotariaAey_20210705.csv", sep=',', dec='.', header=T, as.is=F)
dim(db2)
names(db2)
summary(db)
...
dim(db2_reduced)
names(db2_reduced)
db2_reduced <- db2_reduced[complete.cases(db2_reduced[,29]), ]
dim(db2_reduced)
Amelia::missmap(db2_reduced)
...
names(db2_reduced)
dev.off()
Hmisc::hist.data.frame(db2_reduced[,c(8:13,24:29)])
dev.off()

# transform in log scale CONDUCTIVITY and Chla
summary(db2_reduced)
...
PerformanceAnalytics::chart.Correlation(db2_reduced[,c("PREC",
                              ...
                              "Phycocyanin_BGA.PC")])
# Not collinear:
# CONDUCTIVITY, pH, DO, TEMPERATURE, MEAN_AIR_TEMP, 
...
db2_analyses <- data.frame(lat = db2_reduced$LATITUDE,
                          ...
                          hab =  db2_reduced$hab,
                          conductivity = db2_reduced$CONDUCTIVITY,
                          DO = db2_reduced$DO,
                          ...
                          monogononta = db2_reduced$Monogononta,
                          cladocera = db2_reduced$Cladocera,
                          copepoda = db2_reduced$Copepoda)
...
# prepare the final figure -------------------------------------------------
pdf("Figure productivity.pdf",width=14, height=7, paper='special')
gridExtra::grid.arrange(p_nf2, p_nf3, ncol=2)
dev.off()

pdf("Figure 1.pdf",width=14, height=7, paper='special')
...
for(i in 1:nlevels(db_niche$species)){
  ...
  newHv <- hypervolume::hypervolume_gaussian(na.omit(db_sp)[,c(2:8)],  name= levels(db_niche$species)[i],
                                             kde.bandwidth = 2*estimate_bandwidth(na.omit(db_sp)[,c(2:8)]),
                                             verbose=FALSE)
...
plot(hvBoth,
     ...
     cex.legend=1,
     cex.axis=1,
     col = c("brown4","blue","black","cadetblue4","dodgerblue3","firebrick4","darkmagenta"),
     contour.filled=T,
     contour.filled.alpha=1,
     show.centroid=TRUE,
     cex.centroid=2,
     names= c("Conductivity","Dissolved oxygen","Temperature","pH","Air temperature","Chlorophyll_a", "Phycocyanin"))
dev.off()
